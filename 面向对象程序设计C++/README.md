...
- 4
```
new 的功能是__分配内存空间，然后调用构造函数初始化对象__，
delete 的功能是_调用析构函数析构对象，然后释放内存空间__。
```
- 5
```
...
- 1
```
在C++语言中，使用class 定义类时，隐含的访问权限为___A______。
(A) private （B）protected （C）public （D）const
```
- 2
```
一个类的数据成员可被本类的成员函数访问，该数据成员的权限可以是____D_____。
（A）public （B）protected （C）private （D）以上都对
```
- 3
```
一个只能被本类的成员函数和其友元函数访问的数据成员，其权限是___C______。
（A）public （B）protected （C）private （D）protected 和private
```
- 4
```
在私有继承方式下，派生类只能访问其基类的___D______。
（A）私有成员（B）保护成员（C）公有成员（D）公有和保护成员
```
- 5
```
下列说法中不正确的是___C______。
...
- 7
```
说明一个函数为虚函数时，使用的前缀关键字是___C______。
（A）const （B）static （C）virtual （D）friend

```
- 8
```
在C++中，用于实现动态多态性的是___A______。
（A）虚函数（B）友元函数（C）模板函数（D）静态成员函数

```
- 9
```
下列关于运算符重载的描述中，错误的是___B______。
（A）运算符重载不能改变操作数个数（B）运算符重载不能改变运算符的功能
（C）运算符重载不可以改变结合方向（D）运算符重载不可以改变运算优先级
```
- 10
```
下列特性中不是面向对象程序设计语言所特有的是__D_______。
（A）封装（B）继承（C）多态性（D）模板
```
### （三）判断题
- 1 
```
在C++语言中，在main()函数中，语句const int x; 没有语法错误。(F )
```

- 2
```
在C++语言中，在main()函数中，语句const char *p; 没有语法错误。(T )
```

- 3 
```
在一个类的定义中，可以用语句const int x; 定义一个常数据成员，但必须在该类的所有的
构造函数成员初始值设定项列表中初始化。( T)
```

- 4 
```
在一个类的定义中，可以用语句static int x=0; 定义一个静态数据成员。( F)
```

- 5 
```
在一个类A 中，有带参数的构造函数，但没有无参数的构造函数时，可以使用如下方式定义一
个对象指针： A *p = new A[10]; (F )
```

- 6 
```
设A 类中有公有函数A & operator=(A &t)。在主程序中已有A 类对象obj1，现定义另一个该
类对象obj2 时，使用语句A obj2 = obj1; 该语句会调用A 类的operator= 函数。( F)
```

- 7 
```
设A 类中有公有函数A & operator=(A &t)。在主程序中已有A 类对象obj1,obj2，语句obj2
= obj1; 会调用A 类的operator= 函数。(T )
```

- 8 
```
设A 类中没有以对象引用为参数的拷贝构造函数，在主程序中已有A 类对象obj1，现定义另一
个该类对象obj2 时，语句A obj2 = obj1; 语法是正确的。(T )
```

- 9 
```
下面三种情况下，都会调用以对象引用为参数的拷贝构造函数。(T )
