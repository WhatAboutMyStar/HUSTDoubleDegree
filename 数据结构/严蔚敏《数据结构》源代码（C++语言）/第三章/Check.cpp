...
{   //construct  a empty stack                                               
    S.base=(SElemType *)malloc(STACK_INIT_SIZE * sizeof(SElemType)); // 创建栈
	if(!S.base)
	{  
		cout<<"OVERFLOW!"<<endl;
	    return (ERROR);
	...
	S.top=S.base; // 空栈
	S.stacksize=STACK_INIT_SIZE; // 栈大小
...
{   //Push element to SqStack                                                 
	...
	{  
		S.base=(SElemType *)realloc(S.base,(S.stacksize+STACKINCREMENT*sizeof(SElemType))); // 调整栈空间
		if(!S.base) 
		{ 
			cout<<"Failure to reallocate the Memory units!"<<endl;
		    return(0);
		}
...
{                                                   //if empty(S) then Error,otherwise Delete the S.top element                                                //and return it by variable e
	if(S.top==S.base)
	{ 
		cout<<"Underflow!"<<endl;
		return (ERROR);
	...
	{  
		...
		if(ch==')')
		{
			...
			if(GetTop(S)=='#')                   //get the top element of stack and compare to '#'
				...
				Pop(S,temp);                    //if have '(' to match then Delete '('
		}
		ch=getchar();
	}  //end of while                                         
