...
int StrInsert(HString &S,int pos,HString T)      //StrInsert() function
{
    ...
    if(T.length) 
    { 
		if(!(S.ch=(char*)realloc(S.ch,(S.length+T.length)*sizeof(char)))) // 调整串空间
		{ 
			cout<<"OVERFLOW";
	        return (0);
		}
	    for(i=S.length-1;i>=pos-1;--i) // 移动串(串S中pos位置开始，移动T.length长度)以备插入串T
			S.ch[i+T.length]=S.ch[i];    //ready for inserting HString T
	    for(i=0;i<=T.length-1;i++) // pos位置插入串T
			S.ch[pos-1+i]=T.ch[i];
	    S.length+=T.length;
	    S.ch[S.length]=NULL;
    }//end of if
